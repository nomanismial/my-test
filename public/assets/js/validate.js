...
function check_validate(field, roles, parentEle){
	...
	var messages = {
		...
		"cnic" : "The :attribute is invalid format.",
		"emailphone" : "The email/phone is invalid format.",
		"emailphonereq" : "The email/phone is required.",
		"match" : "The repeat password does not match with :other.",
		...
		$.each(roles, function(key, value){
			switch(key){
				...
				case  "phone":
					...
					break;
				case  "cnic":
					if (is_cnic(field)===true){
						_validate_messages(field, "cnic");
					}
					break;
				case  "emailphone":
	...
	if(typeof errors[0] !== 'undefined') {
		if(type == 'radio'){
			var label_f = field.closest('.icon-group').find('label:first').next('.text');
		}else{
			var label_f = field.closest('.input-box').find('._errorText');
		}
		field.closest('.form-group').addClass('input-group-error');
		field.closest('.form-group').removeClass('input-group-success');
		label_f.html(errors[0]);
		if(type !== undefined){
			if(field_name != 'password'){
				if(type != 'password'){
					if(type != "radio"){
						field.next('.icon').addClass('icon-error');
						field.next('.icon').removeClass('icon-success');
						field.next('.icon').html("<i class='icon-cancel-circle'></i>");
						field.next('.toggle-icon').remove();
					}
				}
			}
		}
		...
		field.closest('.form-group').removeClass('input-group-error');
		field.closest('.form-group').addClass('input-group-success');
		if(type == 'radio'){
			var label_f = field.closest('.icon-group').find('label:first').next('.text');
		}else{
			var label_f = field.closest('.input-box').find('._errorText');
		...
		if(type !== undefined){
			if(field_name != 'password'){
				if(type != 'password'){
					if(type != "radio"){
						field.next('.icon').removeClass('icon-error').addClass('icon-success');
						field.next('.icon').html("<i class='icon-check_circle_outline'></i>");
						field.next('.toggle-icon').remove();
					}
				}
			}
