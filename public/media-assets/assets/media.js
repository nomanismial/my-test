...
$(document).ready(function(){
	...
	$(document).on("change", "#mediaUpload", function(){
		var v = $(this).val();
		var validExts = new Array(".png",".jpg",".jpeg",".gif",".bmp");
		var fileExt = v;
	...
	function upload_u_file(){
		...
		$("#f_upload_form").ajaxForm({
			beforeSend:function(){
				$("<div class='img-ld' style='text-align:center;width:100%;margin-top:30px;'><img src="+gif_img+" style='width:auto;display:inline-block;'><br><span class='__percent_val'>0%</span></div>").insertBefore(".upload-f");
			},
	...
	function data_for_editor(){
		if (window.type=="image"){
			...
			if (window.return!="" && window.return!="undefined"){
				var mediaInfo = $(".f-info .dimension").html();
				mediaInfo = mediaInfo.split("x");
				var width = mediaInfo[0];
				var height = mediaInfo[1];

				var target = $(window.return).get(0).tagName;
				var img = "<img src='"+link+"' "+ tl +" "+ al +"  height = '"+height+"' width='"+width+"'>";
	...
	$(document).on("click", ".___loadMore", function(){
		...
		if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
			if (load_data==true){
				...
				var search = $(".search_images").val();
				var obj = {"f":folder, "page":track_page,"_token":_token,"search":search};
				loading = true; 
